...
#	from Meyer, Cheng, and Knutson (2022) 									
#																	
# Created:  07/25/2018												
# Modified: 11/09/2022												
#																	
...
#### Penalized Bayesian Linear Probability Model ####
pblpm	<- function(X1, X2, B, burnin = NULL, penalty = c('ri', 't', 'la'), ptype = c('l2', 'pe', 'tn'), 
...
coef.pblpm		<- function(mod, CI = FALSE, ci.alpha = 0.05){
  dmpTable	<- apply(mod$rho, 2, quantile, probs = c(0.5, ci.alpha/2, (1 - ci.alpha/2)))
...
pprob.pblpm	<- function(mod, rho0 = 0){
  probs	<- apply(mod$rho > rho0, 2, mean)
  
  return(probs)
}

print.pblpm		<- function(mod){
  K		<- ncol(mod$data$X)/2
...
summary.pblpm	<- function(mod, ci.alpha = 0.05, chains = 4){
  K		<- ncol(mod$data$X)/2
  n		<- nrow(mod$data$X)/(2*K)
  B		<- mod$mcmc.specs$B
  burnin	<- mod$mcmc.specs$burnin
