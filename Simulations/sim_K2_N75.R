...
for(d in 1:ifelse(sparse, length(theta12), length(theta21))){
  ...
  for(m in 1:M){
    ...
    ## bias ##
    # rho #
    br1i75s[m,,d]   <- (tb[1] - tb[3]) - c(median(bspamt$rho[,1]), median(bayhc$rho[,1]), lcRho1, geek$rho[1], boot$resMat[1,1])
    br2i75s[m,,d]   <- (tb[2] - tb[4]) - c(median(bspamt$rho[,2]), median(bayhc$rho[,2]), lcRho2, geek$rho[2], boot$resMat[2,1])
    ...
    cr1i75s[m,,d]   <- 1*c((bspamti1[1] < tb[1] - tb[3] & bspamti1[2] > tb[1] - tb[3]),
                           (bayhci1[1] < tb[1] - tb[3] & bayhci1[2] > tb[1] - tb[3]),
    ...
    cr2i75s[m,,d]   <- 1*c((bspamti2[1] < tb[2] - tb[4] & bspamti2[2] > tb[2] - tb[4]),
                           (bayhci2[1] < tb[2] - tb[4] & bayhci2[2] > tb[2] - tb[4]),
